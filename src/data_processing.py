...
Cost source: data/actual_fantasy_drivers.csv and data/actual_fantasy_constructors.csv,
generated by src/convert_numbers.py from the Numbers spreadsheets in data/numbers/.
"""
...
# FastF1 data is unreliable before this year; skip to avoid hammering the API
FASTF1_MIN_YEAR = 2018

# Seconds to sleep between FastF1 session loads to avoid rate-limiting
FASTF1_REQUEST_DELAY = 1.0
...
def enrich_with_fastf1(driver_df: pd.DataFrame) -> pd.DataFrame:
    """
    For each (year, round) in driver_df at or after FASTF1_MIN_YEAR, fetch
    FP1/FP2/FP3 stats from FastF1 and fill in lap_time_median_gap_fp* and
    lap_time_iqr_fp* columns.  Merges on driver_number.

    A FASTF1_REQUEST_DELAY second sleep is inserted between every session load
    to avoid triggering the FastF1 server rate limit.  Sessions already present
    in the local cache (.fastf1/) are served from disk and do not count against
    the rate limit, so the delay only matters on a cold cache.
    """
    rounds = [
        (int(y), int(r))
        for (y, r) in driver_df.groupby(['year', 'round']).groups
        if int(y) >= FASTF1_MIN_YEAR
    ]
    for year, round_num in rounds:
        for session_type, suffix in [('FP1', 'fp1'), ('FP2', 'fp2'), ('FP3', 'fp3')]:
            stats = fetch_fastf1_session_stats(year, round_num, session_type)
            time.sleep(FASTF1_REQUEST_DELAY)
            if stats.empty:
                continue
...
def enrich_with_costs(driver_df: pd.DataFrame,
                      constructor_df: pd.DataFrame) -> tuple:
    """
    Join costs from actual_fantasy_drivers.csv and actual_fantasy_constructors.csv
    (generated by src/convert_numbers.py from the Numbers spreadsheets).
    Rows without a matching cost entry are left as NaN.
    """
    project_root = Path(__file__).resolve().parent.parent
    data_dir = project_root / "data"

    d_actual_path = data_dir / "actual_fantasy_drivers.csv"
    c_actual_path = data_dir / "actual_fantasy_constructors.csv"

    if d_actual_path.exists():
        d_costs = (
            pd.read_csv(d_actual_path)[['year', 'round', 'driver_id', 'cost']]
            .dropna(subset=['cost'])
        )
        driver_df = driver_df.drop(columns=['cost'], errors='ignore')
        driver_df = pd.merge(driver_df, d_costs,
                             on=['year', 'round', 'driver_id'], how='left')
    else:
        print("  WARNING: actual_fantasy_drivers.csv not found — costs will be NaN")

    if c_actual_path.exists():
        c_costs = (
            pd.read_csv(c_actual_path)[['year', 'round', 'constructor_id', 'cost']]
            .dropna(subset=['cost'])
        )
        constructor_df = constructor_df.drop(columns=['cost'], errors='ignore')
        constructor_df = pd.merge(constructor_df, c_costs,
                                  on=['year', 'round', 'constructor_id'], how='left')
    else:
        print("  WARNING: actual_fantasy_constructors.csv not found — costs will be NaN")
...
def main(window: int = DEFAULT_WINDOW):
    ...
    driver_df, constructor_df = enrich_with_costs(driver_df, constructor_df)
